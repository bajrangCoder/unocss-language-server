...
export async function isPug(uno: UnoGenerator, code: string, id = "") {
  ...
  if (!pugExtractor) return { pug: false, code: "" };
...
export function getPlainClassMatchedPositionsForPug(
  codeSplit: string,
  matchedPlain: Set<string>,
  start: number,
) {
  ...
  matchedPlain.forEach((plainClassName) => {
    ...
    if (plainClassName.includes(":")) {
      ...
      const regex = new RegExp(
        `\.(${plainClassName})[\.#=\s(]|\.(${plainClassName})$`,
      );
...
export function getMatchedPositions(
  ...
  attributify.forEach(([, name, value]) => {
    const regex = new RegExp(
      `(${escapeRegExp(name)}=)(['"])[^\\2]*?${escapeRegExp(value)}[^\\2]*?\\2`,
      "g",
    );
    Array.from(code.matchAll(regex)).forEach((match) => {
      ...
      let bodyIndex =
        body.match(`[\\b\\s'"]${escapeRegExp(value)}[\\b\\s'"]`)?.index ?? -1;
  ...
  result.push(
    ...extraAnnotations.map(
      (i) => [i.offset, i.offset + i.length, i.className] as const,
    ),
  );

  return result
    .filter(([start, end]) => {
      if (excludeRanges.some(([s, e]) => start >= s && end <= e)) return false;
...
export async function getMatchedPositionsFromCode(
  ...
  const transformers = uno.config.transformers?.filter(
    (i) => !ignoreTransformers.includes(i.name),
  );
  const annotations = [];
  ...
  const result = await uno.generate(pug ? pugCode : s.toString(), {
    preflights: false,
  });
